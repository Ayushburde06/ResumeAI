...
def _enrich_rewrite_with_rag(
    ...
    # Prepend RAG context and critique to the resume text passed to rewrite_resume.
    # rewrite_resume already handles the full prompt — we inject context at the
    # top of the resume_text string so it's visible to the LLM.
    enriched_resume_text = resume_text
    if rag_context or critique_block:
        prefix_parts = []
        if rag_context:
...
def run_agent(
    ...
    try:
        plan = plan_analysis(resume_text, jd_text, model_id=model_id)
        memory.job_title = plan.get("job_title", "")
        memory.seniority = plan.get("seniority", "")
        memory.required_skills = plan.get("primary_stack", [])
        ...
        # Non-fatal: continue without plan data
        plan = {}
    ...
    try:
        job_analysis = analyse_job_description(jd_text, model_id=model_id)
        yield _make_event("jd_analysis", "done", {
            "job_title": job_analysis.get("job_title", ""),
            "required_skills_count": len(job_analysis.get("required_skills", [])),
        })
    except Exception as exc:
        yield _make_event("error", "error", {"message": f"JD analysis failed: {str(exc)[:200]}"})
        return
